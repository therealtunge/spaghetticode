...
static inline uint8_t parse_u8(void *ram, unsigned int ptr) {
	return *((uint8_t*) (ram + ptr));
}
...
int processInstruction(VMstate_t *vm) {
	...
	switch (vm->ram[vm->pc]) {
		...
		case 0x0f: {
			...
			for (int i = 0; i < vm->cur_method_locals; i++) {
				pop_regu32(vm);
			}
			vm->pc = pop_u32(vm);
		...
		case 0xd8: {
			write_register(vm->ram[vm->pc + 1] & 0xf, read_register((vm->ram[vm->pc + 1] & 0xf0) >> 4) + parse_u8(vm->ram, vm->pc + 3));
			vm->pc += 4;
			return 0;
		}
		default: {
